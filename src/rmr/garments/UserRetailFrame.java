 ...
 * @author HP
 */
public class UserRetailFrame extends javax.swing.JFrame {

    /**
     * Creates new form HomePage
     */
    public UserRetailFrame(String contact) {
        initComponents(); 
    ...
    private void initComponents() {
        ...
        IconBar.add(profilelogo, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 250, 50, 47));
        ...
        menu.add(p7, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 250, 280, 47));
        ...
        retailpanel1.add(pcontact, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 10, 0, 0));
...
Connection con;
    ...
    private void retaillogoMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_retaillogoMouseClicked
        UserRetailFrame r=new UserRetailFrame(pfcontact);
        r.setVisible(true);
        this.dispose();
    ...
    private void wholesalelogoMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_wholesalelogoMouseClicked
        UserWholeSaleFrame w=new UserWholeSaleFrame(pfcontact);
        w.setVisible(true);
        this.dispose();
...
int info=0,billno=0;
    private void formWindowGainedFocus(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowGainedFocus
       ...
       if(info==0)
       {
            ...
            Statement st=con.createStatement();
            ResultSet rs1=st.executeQuery("Select distinct(Code) from jeans where Quantity>0");
            while(rs1.next())
            {
                code1.addItem(rs1.getString("Code"));
            }
            ResultSet rs2=st.executeQuery("Select distinct(Code) from trouser where Quantity>0");
            while(rs2.next())
            {
                code2.addItem(rs2.getString("Code"));
            }
            ResultSet rs3=st.executeQuery("Select distinct(Item) from undergarments where Quantity>0");
            while(rs3.next())
            {
                code3.addItem(rs3.getString("Item"));
            }
            ResultSet rs4=st.executeQuery("Select distinct(Item) from socks where Quantity>0");
            while(rs4.next())
            {
                sockscombo.addItem(rs4.getString("Item"));
            }
            ResultSet rs5=st.executeQuery("Select distinct(Code) from halfsleeveshirt where Quantity>0");
            while(rs5.next())
            {
                code5.addItem(rs5.getString("Code"));
            }
            ResultSet rs6=st.executeQuery("Select distinct(Code) from Fullsleeveshirt where Quantity>0");
            while(rs6.next())
            {
                code6.addItem(rs6.getString("Code"));
            }
            ResultSet rs7=st.executeQuery("Select distinct(Code) from lowers where Quantity>0");
            while(rs7.next())
            {
                code4.addItem(rs7.getString("Code"));
            }
            ResultSet rs8=st.executeQuery("Select distinct(Code) from halfsleevetshirt where Quantity>0");
            while(rs8.next())
            {
                code7.addItem(rs8.getString("Code"));
            }
            ResultSet rs9=st.executeQuery("Select distinct(Code) from fullsleevetshirt where Quantity>0");
            while(rs9.next())
            {
                code8.addItem(rs9.getString("Code"));
            }
            ResultSet rs10=st.executeQuery("Select distinct(Item) from seasonal where Quantity>0");
            while(rs10.next())
            {
...
int profit,data=0,discount=0;
    private void socksadd3MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_socksadd3MouseClicked
        ...
        try
        {
            ...
            while(rs.next())
            {
                discount=rs.getInt("RetailDiscount");
            }
        }
        catch(Exception e)
        {
    ...
    private void socksadd4MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_socksadd4MouseClicked
        ...
        else
        {
           if(receiptcount==0)
            ...
            try{
                Statement st=con.createStatement();
                st.executeUpdate("insert into customerinfo values("+billno+",'"+cname+"','"+cmobile+"','"+caddress+"',"+cage+","+total+","+profit+","+cash+","+online+",'"+purchasedate+"','"+purchasetime+"','"+method+"','Retail')");
...
String str;
    ...
    private void jLabel14MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel14MouseClicked
        RetalDiscountFrame d=new RetalDiscountFrame();
        d.setVisible(true);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(UserRetailFrame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                String a="";
                new UserRetailFrame(a).setVisible(true);
            }
        });
    }
